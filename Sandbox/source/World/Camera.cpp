...
Camera::Camera()
{
    ...
    m_vecCameraLookAt = glm::vec3(0,0,1);

    m_bPrevInitialized = false;
    m_vecPrevMousePos = glm::vec2(0);
    m_vecMousePosition = glm::vec2(gWindowWidht / 2.0f, gWindowHeight / 2.0f);
}
...
void Camera::Update(float dt)
{
    ...
    m_matProjection = glm::perspectiveFov(m_fFOV, (float)gWindowWidht, (float)gWindowHeight, m_fNearClip, m_fFarClip);
